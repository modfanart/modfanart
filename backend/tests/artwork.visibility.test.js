...
test('list and count queries apply an identical visibility filter', () => {
  ...
  assert.deepEqual(visibilityParams(list.parameters), [
    'published',
    'approved',
    'winner',
    'finalized',
...
test('search does not widen visibility via OR', () => {
  ...
  assert.match(
    count.sql,
    new RegExp(`${FINALIZED_WINNER_EXISTS.source} and \\(`)
  );
});
...
test('getArtworks wires the filter into both of its queries', () => {
  ...
  const source = fs.readFileSync(
    path.join(
      __dirname,
      '../src/modules/artworks/controller/artwork.controller.js'
    ),
    'utf8'
  );
  ...
  assert.ok(
    getArtworks.length > 0,
    'could not locate getArtworks in controller source'
  );
...
test('draft, pending, rejected and disqualified can never satisfy the filter', () => {
  ...
  for (const value of ['draft', 'pending', 'rejected', 'disqualified']) {
    assert.ok(
      ...
      `${value} must not appear in the filter`
    );
  }
});
