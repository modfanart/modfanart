...
class AuthController {
  // POST /api/auth/sync
  // Called after every Firebase sign-in. Verifies the Firebase ID token,
  // finds or creates the user in our DB, and returns the DB user record.
  static async sync(req, res) {
    try {
      ...
      // 2. Migration: link an existing email/password user to their Firebase account
      if (!user && email) {
      ...
      // 5. Return user with role + brands
