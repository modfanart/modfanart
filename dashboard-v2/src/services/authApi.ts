...
> = async (args, api, extraOptions) => {
    ...
    if (result.error && (result.error as any).originalStatus === 401) {
        ...
        if (!refreshToken) {
            console.warn(
                '[RTK Query] No refresh token found in localStorage → forcing logout',
            )
            api.dispatch(logout())
        ...
        if (refreshResult.data) {
            ...
            // Option 1: Keep existing user (fastest)
            api.dispatch(
                setCredentials({
                    accessToken,
                    user: (api.getState() as RootState).auth.user ?? null,
                    persist: true,
                }),
            )

            // Option 2: Or force refetch of user profile after refresh (more correct in some apps)
            // api.dispatch(authApi.util.invalidateTags(['CurrentUser']));
        } else {
