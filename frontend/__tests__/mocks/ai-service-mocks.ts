    ...
    choices: [
      {
        message: {
          role: "assistant",
          content: JSON.stringify({
            ipCompliance: { score: 8, issues: ["Direct copy of protected character"], riskLevel: "high" },
            contentSafety: { score: 7, issues: ["Potentially inappropriate content"], riskLevel: "medium" },
            brandGuidelines: {
              adherence: "low",
            ...
            reasoningSummary: "Content appears to infringe on IP and contains inappropriate elements",
          }),
        },
        finish_reason: "stop",
        index: 0,
    ...
    choices: [
      {
        message: {
          role: "assistant",
          content: JSON.stringify({
            ipCompliance: { score: 5, issues: ["Similarity to protected character"], riskLevel: "medium" },
...
export function setupAIMocks(options: {
  ...
  if (aiOrNotResponse === "success") {
    ...
    ;(global.fetch as jest.Mock).mockImplementationOnce(() => mockFetchResponse(500, mockAIOrNotResponse.error()))
  }

  // Mock OpenAI API
  if (openAIResponse === "success") {
    ;(global.fetch as jest.Mock).mockImplementationOnce(() => mockFetchResponse(200, mockOpenAIResponse.success()))
  } else if (openAIResponse === "reject") {
    ;(global.fetch as jest.Mock).mockImplementationOnce(() => mockFetchResponse(200, mockOpenAIResponse.reject()))
  } else if (openAIResponse === "review") {
    ;(global.fetch as jest.Mock).mockImplementationOnce(() => mockFetchResponse(200, mockOpenAIResponse.review()))
  } else if (openAIResponse === "malformed") {
    ;(global.fetch as jest.Mock).mockImplementationOnce(() => mockFetchResponse(200, mockOpenAIResponse.malformed()))
  } else {
    ;(global.fetch as jest.Mock).mockImplementationOnce(() => mockFetchResponse(500, mockOpenAIResponse.error()))
  }
}
