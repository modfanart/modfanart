...
import type {
  ContestData, ContestStatus, Visibility, AIPolicy,
  ContestPrize, TimelineEntry, JudgingCriterion, FAQItem,
} from "./contestTypes";
...
function emptyContest(): Omit<ContestData, "id"> {
  return {
    status:     "draft",
    visibility: "public",
    brand: { name: "", logoInitial: "", logoUrl: null, accentColor: "#2563eb", bgColor: "#f4f6fb", surfaceColor: "#ffffff" },
    heroImageUrl: "", title: "", tagline: "", contestType: "Fan Art Contest",
    region: "Global", ageRequirement: "18+",
    submissionDeadlineDisplay: "", winnersCount: 1, prizeModel: "Cash + Licensing", productFocus: "",
    startDateDisplay: "", submissionDeadlineIso: "", winnersAnnouncedDisplay: "Shortly After Deadline", eligibilityLabel: "Global, 18+",
    prizeSummary: { headline: "", bullets: [""] },
    submissionSnapshot: [""], commercialIntent: [""],
    overviewParagraphs: [""],
    brief: { guidelinesPdfUrl: null, theme: "", styleDirection: "", targetAudience: "", referenceNotes: "", referenceImages: [] },
    requirements: {
      allowed: [""], notAllowed: [""], remember: [""],
      specs: { fileFormats: "PNG, PSD, AI, SVG", minResolution: "300 DPI / 4500 × 5400 px", requiredFields: "Title, description, tags, process notes", mockupRequired: "Optional" },
    },
    submissionRules: { maxEntriesPerArtist: 3, aiPolicy: "disclosure_required", aiPolicyLabel: "Disclose Usage" },
    ...
    legal: { licenseType: "Optional on selection", rightsGranted: "", territory: "Global", aiPolicy: "AI assisted only with disclosure", ageRequirement: "18+", termsUrl: "" },
    faq: [{ question: "", answer: "" }],
    submissionChecklist: ["Original artwork only", "Correct file format and resolution", "Title and description included", "AI disclosure if applicable", "Agree to submission terms"],
    inspirationNote: "Review the creative brief, approved themes, and reference materials before submitting your concept.",
    submitUrl: "", termsUrl: "",
  };
}
    ...
    <div style={{ marginBottom: 18 }}>
      ...
      {values.map((v, i) => (
        <div key={i} style={{ display: "flex", gap: 8, marginBottom: 6 }}>
          ...
          />
          <button type="button" className="ncf-btn-ghost" onClick={() => onChange(values.filter((_, j) => j !== i))} aria-label="Remove">✕</button>
        </div>
      ))}
      <button type="button" className="ncf-btn-add" onClick={() => onChange([...values, ""])}>+ Add item</button>
    </div>
  );
}
...
export default function NewContestForm() {
  ...
  function field(label: string, path: string, opts?: { type?: string; placeholder?: string; textarea?: boolean; rows?: number }) {
    const keys = path.split(".");
    ...
    return (
      <div className="ncf-field">
        <label className="ncf-label">{label}</label>
        {opts?.textarea ? (
          <textarea className="ncf-input ncf-textarea" rows={opts.rows ?? 3} placeholder={opts?.placeholder}
            value={String(val ?? "")} onChange={e => set(path, e.target.value)} />
        ) : (
          <input className="ncf-input" type={opts?.type ?? "text"} placeholder={opts?.placeholder}
            value={String(val ?? "")} onChange={e => set(path, opts?.type === "number" ? Number(e.target.value) : e.target.value)} />
        )}
      </div>
    );
  }
  ...
  function select(label: string, path: string, options: { value: string; label: string }[]) {
    ...
    return (
      <div className="ncf-field">
        <label className="ncf-label">{label}</label>
        <select className="ncf-input ncf-select" value={String(val ?? "")} onChange={e => set(path, e.target.value)}>
    ...
    return (
      <div className="ncf-wrap">
        ...
        <div className="ncf-success">
          ...
          <p>Your contest has been saved as a draft. You can preview and publish it from the dashboard.</p>
          <button className="ncf-btn-primary" onClick={() => { setSubmitted(false); setStep(1); setForm(emptyContest()); }}>
            Create Another
          </button>
        </div>
      </div>
  ...
  return (
    <div className="ncf-wrap">
      ...
      <div className="ncf-header">
        ...
        <p className="ncf-header__sub">Fill in the details below to create your fan-art campaign. You can save as a draft at any time.</p>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <div className="ncf-card">
          ...
          {step === 1 && (
            <>
              ...
              <div className="ncf-grid-2">
                {field("Contest Title *", "title", { placeholder: "Dark Legends Merch Design Contest" })}
                {field("Brand / IP Name *", "brand.name", { placeholder: "Dark Legends" })}
              </div>
              {field("Tagline / Short Description *", "tagline", { textarea: true, rows: 2, placeholder: "Create original artwork inspired by…" })}
              <div className="ncf-grid-2">
              ...
              {field("Hero Image URL *", "heroImageUrl", { placeholder: "https://images.unsplash.com/…" })}
            </>
          )}
          ...
          {step === 2 && (
            <>
              ...
              {field("Theme / Concept *", "brief.theme", { textarea: true, rows: 2, placeholder: "Dark fantasy iconography, ancient factions…" })}
              {field("Style Direction *", "brief.styleDirection", { textarea: true, rows: 2, placeholder: "Premium graphic apparel, bold composition…" })}
              {field("Target Audience *", "brief.targetAudience", { textarea: true, rows: 2, placeholder: "Core fandom collectors, fantasy genre fans…" })}
              {field("Reference Notes", "brief.referenceNotes", { textarea: true, rows: 2, placeholder: "Use official lore, approved symbols…" })}
              <div className="ncf-divider" />
              <p className="ncf-section-label">Brand Guidelines PDF (optional)</p>
              {field("Guidelines PDF URL", "brief.guidelinesPdfUrl", { placeholder: "https://…/brand-guidelines.pdf — leave blank to hide the download banner" })}
            </>
          )}
          ...
          {step === 3 && (
            <>
              ...
              <ArrayField label="✅ Allowed"
                values={form.requirements.allowed}
                onChange={v => set("requirements.allowed", v)} />
              <ArrayField label="🚫 Not Allowed"
                values={form.requirements.notAllowed}
                onChange={v => set("requirements.notAllowed", v)} />
              <ArrayField label="⚠️ Remember"
                values={form.requirements.remember}
                onChange={v => set("requirements.remember", v)} />
              <div className="ncf-divider" />
              <p className="ncf-section-label">File Specs</p>
              <div className="ncf-grid-2">
                {field("Allowed File Formats", "requirements.specs.fileFormats", { placeholder: "PNG, PSD, AI, SVG" })}
                {field("Minimum Resolution", "requirements.specs.minResolution", { placeholder: "300 DPI / 4500 × 5400 px" })}
              </div>
              <div className="ncf-grid-2">
                {field("Required Submission Fields", "requirements.specs.requiredFields", { placeholder: "Title, description, tags, process notes" })}
              ...
              <div className="ncf-grid-2">
                {field("Max Entries Per Artist", "submissionRules.maxEntriesPerArtist", { type: "number", placeholder: "3" })}
              ...
              {field("AI Policy Display Label", "submissionRules.aiPolicyLabel", { placeholder: "Disclose Usage" })}
            </>
          )}
          ...
          {step === 4 && (
            <>
              ...
              <ArrayField label="Product Categories"
              ...
              <div className="ncf-grid-2">
                ...
                {field("Submission Deadline Display", "submissionDeadlineDisplay", { placeholder: "9 April 2026" })}
              </div>
              <div className="ncf-grid-2">
                {field("Submission Deadline (ISO for logic)", "submissionDeadlineIso", { type: "date" })}
                {field("Winners Announced Label", "winnersAnnouncedDisplay", { placeholder: "Shortly After Deadline" })}
              </div>
              ...
              {form.timeline.map((row, i) => (
                <div key={i} className="ncf-grid-2" style={{ alignItems: "flex-end" }}>
                  <div className="ncf-field">
                    <label className="ncf-label">Label</label>
                    <input className="ncf-input" value={row.label}
                      onChange={e => { const t = [...form.timeline]; t[i] = { ...t[i], label: e.target.value } as TimelineEntry; set("timeline", t); }} />
                  </div>
                  <div className="ncf-field">
                    <label className="ncf-label">Date</label>
                    <div style={{ display: "flex", gap: 8 }}>
                      <input className="ncf-input" value={row.date} style={{ flex: 1 }}
                        onChange={e => { const t = [...form.timeline]; t[i] = { ...t[i], date: e.target.value } as TimelineEntry; set("timeline", t); }} />
                      <button type="button" className="ncf-btn-ghost"
                        onClick={() => set("timeline", form.timeline.filter((_, j) => j !== i))}>✕</button>
                    </div>
                  </div>
                </div>
              ))}
              <button type="button" className="ncf-btn-add"
          ...
          {step === 5 && (
            <>
              ...
              {field("Prize Summary Headline", "prizeSummary.headline", { placeholder: "$2,000 Prize Pool + Official Licensing Opportunity" })}
              <ArrayField label="Prize Summary Bullets"
                ...
                <div key={i} className="ncf-prize-row">
                  <div className="ncf-grid-3" style={{ flex: 1 }}>
                    <div className="ncf-field">
                      ...
                      <input className="ncf-input" value={prize.place}
                        onChange={e => { const p = [...form.prizes]; p[i] = { ...p[i], place: e.target.value } as ContestPrize; set("prizes", p); }} />
                    </div>
                    <div className="ncf-field">
                      <label className="ncf-label">Amount</label>
                      <input className="ncf-input" value={prize.amount} placeholder="$1,000"
                        onChange={e => { const p = [...form.prizes]; p[i] = { ...p[i], amount: e.target.value } as ContestPrize; set("prizes", p); }} />
                    </div>
                    <div className="ncf-field" style={{ gridColumn: "span 2" }}>
                      <label className="ncf-label">Description</label>
                      <input className="ncf-input" value={prize.description}
                        onChange={e => { const p = [...form.prizes]; p[i] = { ...p[i], description: e.target.value } as ContestPrize; set("prizes", p); }} />
                    </div>
                  </div>
                  <button type="button" className="ncf-btn-ghost ncf-prize-del"
                    onClick={() => set("prizes", form.prizes.filter((_, j) => j !== i))}>✕</button>
                </div>
              ))}
              <button type="button" className="ncf-btn-add"
                onClick={() => set("prizes", [...form.prizes, { place: "", amount: "", description: "" }])}>
                + Add Prize Tier
              </button>
              <div className="ncf-divider" />
              {field("Licensing Opportunity Note", "licensingOpportunityNote", { textarea: true, rows: 3 })}
              <div className="ncf-divider" />
              <ArrayField label="Submission Snapshot (sidebar card)"
                values={form.submissionSnapshot}
                onChange={v => set("submissionSnapshot", v)} />
              <ArrayField label="Commercial Intent (sidebar card)"
              ...
              {form.judging.criteria.map((c, i) => (
                <div key={i} className="ncf-grid-2" style={{ alignItems: "flex-end" }}>
                  <div className="ncf-field">
                    ...
                    <input className="ncf-input" value={c.title}
                      onChange={e => { const cr = [...form.judging.criteria]; cr[i] = { ...cr[i], title: e.target.value } as JudgingCriterion; set("judging.criteria", cr); }} />
                  </div>
                  <div className="ncf-field">
                    <label className="ncf-label">Description</label>
                    <div style={{ display: "flex", gap: 8 }}>
                      <input className="ncf-input" value={c.description} style={{ flex: 1 }}
                        onChange={e => { const cr = [...form.judging.criteria]; cr[i] = { ...cr[i], description: e.target.value } as JudgingCriterion; set("judging.criteria", cr); }} />
                      <button type="button" className="ncf-btn-ghost"
                        onClick={() => set("judging.criteria", form.judging.criteria.filter((_, j) => j !== i))}>✕</button>
                    </div>
                  </div>
                </div>
              ))}
              <button type="button" className="ncf-btn-add"
                onClick={() => set("judging.criteria", [...form.judging.criteria, { title: "", description: "" }])}>
                + Add Criterion
              </button>
              <div className="ncf-grid-2" style={{ marginTop: 16 }}>
                {field("Selection Type", "judging.selectionType", { placeholder: "Internal Brand and MOD Review" })}
                {field("Review Team", "judging.reviewTeam", { placeholder: "Brand Team, MOD Licensing Team…" })}
              </div>
              <div className="ncf-divider" />
              <p className="ncf-section-label">Legal Details</p>
              <div className="ncf-grid-2">
                {field("License Type", "legal.licenseType", { placeholder: "Optional on selection" })}
                {field("Territory", "legal.territory", { placeholder: "Global" })}
              </div>
              {field("Rights Granted If Selected", "legal.rightsGranted", { textarea: true, rows: 2 })}
              <div className="ncf-grid-2">
                {field("AI Policy (legal display)", "legal.aiPolicy", { placeholder: "AI assisted only with disclosure" })}
            ...
            <>
              ...
              {form.faq.map((item, i) => (
                <div key={i} className="ncf-faq-row">
                  <div className="ncf-field" style={{ flex: 1 }}>
                    ...
                    <input className="ncf-input" value={item.question}
                      onChange={e => { const f = [...form.faq]; f[i] = { ...f[i], question: e.target.value } as FAQItem; set("faq", f); }} />
                  </div>
                  <div className="ncf-field" style={{ flex: 1 }}>
                    <label className="ncf-label">Answer</label>
                    <div style={{ display: "flex", gap: 8 }}>
                      <textarea className="ncf-input ncf-textarea" rows={2} style={{ flex: 1 }} value={item.answer}
                        onChange={e => { const f = [...form.faq]; f[i] = { ...f[i], answer: e.target.value } as FAQItem; set("faq", f); }} />
                      <button type="button" className="ncf-btn-ghost"
                        onClick={() => set("faq", form.faq.filter((_, j) => j !== i))}>✕</button>
                    </div>
                  </div>
                </div>
              ))}
              ...
              {form.overviewParagraphs.map((p, i) => (
                <div key={i} style={{ display: "flex", gap: 8, marginBottom: 8 }}>
                  <textarea className="ncf-input ncf-textarea" rows={3} style={{ flex: 1 }} value={p}
                    onChange={e => { const ps = [...form.overviewParagraphs]; ps[i] = e.target.value; set("overviewParagraphs", ps); }} />
                  <button type="button" className="ncf-btn-ghost"
                    onClick={() => set("overviewParagraphs", form.overviewParagraphs.filter((_, j) => j !== i))}>✕</button>
                </div>
              ))}
              <button type="button" className="ncf-btn-add"
              ...
              <div className="ncf-divider" />
              <ArrayField label="Submission Checklist (sidebar)"
              ...
              <div className="ncf-review-box">
                ...
                <p>{form.brand.name ? `Hosted by ${form.brand.name}` : "No brand name entered yet"}</p>
                <p>{form.submissionDeadlineDisplay ? `Deadline: ${form.submissionDeadlineDisplay}` : "No deadline set"}</p>
              </div>
            </>
          )}
        </div>
        ...
        <div className="ncf-nav">
          {step > 1 && (
            <button type="button" className="ncf-btn-secondary" onClick={prev}>← Previous</button>
          )}
          <div style={{ marginLeft: "auto", display: "flex", gap: 10 }}>
            {step < STEPS.length ? (
              <button type="button" className="ncf-btn-primary" onClick={next}>Next →</button>
            ) : (
              <>
                <button type="button" className="ncf-btn-secondary"
                  onClick={() => { set("status", "draft"); handleSubmit({ preventDefault: () => {} } as React.FormEvent); }}>
                  Save as Draft
                </button>
                <button type="submit" className="ncf-btn-primary"
