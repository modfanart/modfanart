...
describe('public results page - a viewer with no account', () => {
  ...
  it('degrades cleanly when the API predates the new fields', async () => {
    // Vercel ships this page on merge; the backend waits for a manual
    // redeploy. In that window the payload has no submission_notes,
    // artwork_description, or artwork_id. The page must render the winner
    // without an Artist's note box and without any /artwork/undefined link.
    fetchResponse = jsonResponse(200, {
      contest: { title: 'Skew Contest', hero_image: null },
      winners: [
        {
          entry_id: 'e9',
          rank: 1,
          artwork_title: 'Old Payload Piece',
          artwork_thumbnail: null,
          artwork_file_url: 'https://cdn/9.png',
          creator_username: 'artist_nine',
        },
      ],
    });
    const container = await render(React.createElement(PublicResultsPage));
    ...
    const text = container.textContent;
    assert.ok(text.includes('Old Payload Piece'), text);
    assert.ok(!text.includes("Artist's note"), 'no note box for a payload without notes');

    const hrefs = [...container.querySelectorAll('a')].map((a) => a.getAttribute('href'));
    assert.ok(
      !hrefs.some((h) => h && h.includes('undefined')),
      `no link may point at undefined, got ${hrefs}`
    );
    const srcs = [...container.querySelectorAll('img')].map((i) => i.getAttribute('src'));
    assert.ok(srcs.includes('https://cdn/9.png'), 'thumbnail must still render unlinked');
  });
