...
export async function isRequestAuthenticated(request: NextRequest): Promise<boolean> {
  try {
    ...
    if (shouldBypassAuth()) {
      logger.debug('Authentication bypassed due to BYPASS_AUTH', {
    ...
    try {
      ...
      logger.debug('Token exists (no verification in demo mode)', {
        path: request.nextUrl.pathname,
      });
      return true;
    } catch (error) {
...
export async function requestHasRole(request: NextRequest, role: string): Promise<boolean> {
  try {
    ...
    if (shouldBypassAuth()) {
      logger.debug('Role check bypassed due to BYPASS_AUTH', {
        role,
        path: request.nextUrl.pathname,
      });
      return true;
    ...
    const token = request.cookies.get(AUTH_COOKIE_NAME)?.value;
    if (!token) {
      logger.debug('No auth token found in cookies for role check', {
        role,
        path: request.nextUrl.pathname,
      });
      return false;
