...
const RoleBadge = ({ role }) => {
  ...
  return (
    <span
      className={`badge ${roleColors[role] || 'border-zinc-700 text-zinc-400'
        } bg-zinc-950`}
    >
      {role?.replace('_', ' ')}
    </span>
...
export const UsersPage = () => {
  ...
  const handlePageChange = (page) => {
    if (
      page < 1 ||
      page > userPagination.total_pages ||
      page === userPagination.page
    ) {
      return;
    }

    fetchUsers(page, userPagination.limit);
  };

  const handleLimitChange = (limit) => {
    fetchUsers(1, Number(limit));
  };
  ...
  const handleUpdateUser = async (updates) => {
    try {
      ...
      fetchUsers(userPagination.page, userPagination.limit);
    } catch (error) {
      toast.error(
  ...
  const handleDeleteUser = async () => {
    try {
      ...
      const nextPage =
        users.length === 1 && userPagination.page > 1
          ? userPagination.page - 1
          : userPagination.page;

      fetchUsers(nextPage, userPagination.limit);
    } catch (error) {
      toast.error(
  ...
  const getPageNumbers = () => {
    const { page, total_pages } = userPagination;
                ...
                <>
                          ...
                          >
                            ...
                            <td>
                              <button
                                ...
                                className={`p-1 rounded ${user.email_verified
                                  ? 'text-green-500'
                                  : 'text-red-500'
                                  } ${canEditUsers
                                    ? 'hover:bg-zinc-800 cursor-pointer'
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-4 py-4 border-t border-zinc-800">
                    ...
                    <div className="flex items-center gap-4 text-sm text-zinc-500">
                      ...
                      >
                        <SelectTrigger className="h-8 w-[80px] bg-zinc-950 border-zinc-800 text-zinc-300">
                          <SelectValue />
                        </SelectTrigger>
                        ...
                        {getPageNumbers().map(
                          (pageNumber, index) =>
                            pageNumber === '...' ? (
                              <span
                                key={`ellipsis-${index}`}
                                className="flex h-8 w-8 items-center justify-center text-zinc-600 text-sm"
                              >
                                ...
                              </span>
                            ) : (
                              <Button
                                ...
                                variant="ghost"
                                size="sm"
                                className={`h-8 min-w-8 px-2 ${pageNumber ===
                                  userPagination.page
      ...
      >
        <DialogContent className="bg-zinc-900 border-zinc-800">
          ...
          <p className="text-zinc-400">
            ...
            <span className="text-white">
              {deleteRoleConfirm.role?.display_name}
            </span>
            ? This action cannot be undone.
          </p>
...
const RolePermissionsModal = ({
  ...
  const filteredModules = Object.keys(
    permissionsByModule
  ).filter((module) => {
    ...
    return permissionsByModule[module].some(
      (p) =>
                ...
                >
                  ...
                  >
                    ...
                    <span className="text-xs text-zinc-500">
                      (
                      {
                        permissionsByModule[module]?.filter(
                          (p) =>
                  ...
                  <div className="p-2 space-y-1">
                    {permissionsByModule[module]?.map(
                      (perm) => (
                        <div
                          key={perm.id}
                          className={`flex items-center gap-3 p-2 rounded ${isEditing
                        ...
                        >
                          {isEditing ? (
                            ...
                            <div
                              className={`w-4 h-4 rounded flex items-center justify-center ${selectedPermissions.includes(
                                perm.id
                              )
          ...
          <div className="space-y-2">
            ...
            <ScrollArea className="h-[300px] border border-zinc-800 rounded-md p-2">
              <div className="space-y-3">
                {Object.keys(permissionsByModule).map(
                  (module) => (
                    <div key={module}>
