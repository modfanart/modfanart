      ...
      {requests.map((req) => (
        <div key={req.id} className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
          <div className="flex justify-between items-start">
            <div>
              <h3 className="font-semibold text-white text-lg">{req.company_name}</h3>
              <p className="text-zinc-400">{req.contact_email}</p>
            </div>
          ...
          <div className="flex gap-3 mt-6">
            <Button onClick={() => onApprove(req.id)} className="flex-1">
              Approve Verification
            </Button>
            <Button variant="outline" className="flex-1 border-red-900 text-red-400 hover:bg-red-950">
              Reject
            </Button>
          </div>
        </div>
